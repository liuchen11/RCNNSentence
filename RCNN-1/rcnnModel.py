...
class RCNNModel(object):
	...
	def train_validate_test(self,trainSet,validateSet,testSet,nEpoch):
		...
		#f=theano.function([index],[T.dot(T.concatenate(self.layer1Inputs,1),self.layer1.w),T.concatenate(self.layer1Inputs,1),self.layer1.w],
		#givens={
	...
	def save(self):
		...
		fileName=str(timeStruct.tm_mon)+'_'+str(timeStruct.tm_mday)+'_'+str(timeStruct.tm_hour)+'_'+str(timeStruct.tm_min)+'__'+str(self.result['finalAcc'])
