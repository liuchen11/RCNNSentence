...
def parseConfig(sentences,vocab,config,vectors,wordIndex,indexWord,static,useVal,name,load):
	...
	>>>type useVal:bool
	>>>para useVal:whether or not to use a validation set
	>>>type name:str
	>>>para name:the name of the model
	>>>type load:str
	>>>para load:a file containing the network
	'''
	...
	if cross==False:
		...
		if len(validation)==0 and useVal:				#No ValidationSet
			...
			trainSetY=newTrainSetY;validationSetY=newValidationSetY
                ...
                if not useVal and len(validation)!=0:
		...
		try:
			predictInfo,precision=network.train_validate_test(trainSet,validateSet,testSet,25)
		except:
		...
		cPickle.dump(network,open('../Nets/'+fileName,'wb'))
                predictInfo['testSet']=testSet
                predictInfo['indexWord']=indexWord
                cPickle.dump(predictInfo,open('../Errors/'+fileName,'wb'))
		...
		for item in sets:
			...
			precisions.append(precision)
                        testPredicts.append(predictInfo)
...
if __name__=='__main__':
	...
	for i in xrange(len(sys.argv)):
        ...
        print 'config: dataFile:%s, vecFile:%s, useVal:%r, static:%r,rand:%r'%(dataFile,vecFile,useVal,static,rand)
